...
#ifndef __ENCRYPT_HPP_
#define __ENCRYPT_HPP_

#ifdef __linux__
	// skCrypter.h:64:3 error: unknown type name '__forceinline'
...
namespace kuz {
	...
	static constexpr unsigned char key_size = 32;
	static constexpr unsigned char num_keys = 16;

	using block_t = std::array<BYTE, block_size>;
	using key_pair = std::array<block_t, 2>;
	using key_t = std::array<BYTE, key_size>;

	class Kuznechik final {
	public:
		Kuznechik(const std::wstring&);
		BOOST_DEFAULTED_FUNCTION(~Kuznechik(), { })
		BOOST_DELETED_FUNCTION(Kuznechik(Kuznechik&&))
		BOOST_DELETED_FUNCTION(Kuznechik(const Kuznechik&))
		BOOST_DELETED_FUNCTION(Kuznechik& operator=(const Kuznechik&))
		BOOST_DELETED_FUNCTION(Kuznechik& operator=(Kuznechik&&))

		BOOST_ATTRIBUTE_NODISCARD std::string encrypt(const std::wstring&) const;
		BOOST_ATTRIBUTE_NODISCARD std::wstring decrypt(const std::wstring&) const;
		BOOST_ATTRIBUTE_NODISCARD std::wstring decrypt(const std::string&) const;


	private:
		block_t p_First_Gamma;
		std::vector<key_pair> p_Key_Pairs;
		std::array<block_t, num_keys> p_Keys;

		BOOST_ATTRIBUTE_NODISCARD std::vector<key_pair> getRoundKeys(const key_t&) const;
		static block_t increment(const block_t&);
		static block_t X(const block_t&, const block_t&);
		BOOST_ATTRIBUTE_NODISCARD std::array<block_t, 2> f(const block_t&, const block_t&, const block_t&) const;
		BOOST_ATTRIBUTE_NODISCARD block_t r_inv(const block_t&) const;
		BOOST_ATTRIBUTE_NODISCARD block_t l_inv(const block_t&) const;
		BOOST_ATTRIBUTE_NODISCARD block_t L(const block_t&) const;
		BOOST_ATTRIBUTE_NODISCARD block_t R(const block_t&) const;
		BOOST_ATTRIBUTE_NODISCARD block_t S_inv(const block_t&) const;
		BOOST_ATTRIBUTE_NODISCARD block_t S(const block_t&) const;
		BOOST_ATTRIBUTE_NODISCARD block_t lsx(const block_t&, const block_t&) const;
		static BYTE gf_Mul(BYTE, BYTE);
